...
let find_trait_opt (env : t) (name : string) =
  match Hash_string.find_opt env.traits name with
  | None -> None
  | Some trait_info -> Some trait_info.decl
...
let make ~foreign_types ~type_defs ~trait_defs ~trait_alias =
  ...
  { types = type_defs; traits; newtype_deps; type_alias_deps; trait_alias }
...
let iter_traits (env : t) (f : string -> Syntax.trait_decl -> unit) =
  (Hash_string.iter2 env.traits (fun name trait_info -> f name trait_info.decl) : unit)
...
let traits_to_list_map (type a) (env : t) (f : string -> Syntax.trait_decl -> a)
    =
  (Hash_string.to_list_with env.traits (fun name trait_info -> f name trait_info.decl) : a list)
