...
let convert_single name body terminator (inst: Riscv_ssa.t) =
  ...
  | Branch { cond; ifso; ifnot } ->
      terminator := Term.Beq { rs1 = slot_v cond; rs2 = Slot.Reg Zero; ifso; ifnot }

  | Jump label ->
      terminator := Term.J label
...
let gen_fn (f: fn) =
  ...
  List.iter (fun x ->
    ...
    Vec.push vblocks {
      body; term = !term; preds = block.pred
    }
  ) blocks;
  ()
...
let virtasm_of_ssa (ssa : Riscv_ssa.t list) =
  ...
  ({
    ...
    consts = Hashtbl.create 1;
    loop_vars = Hashtbl.create 1;
  }: VProg.t)
