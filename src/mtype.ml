...
include struct
  ...
  let rec sexp_of_t t = match t with
    ...
    | T_optimized_option { elem } ->
        S.List [ S.Atom "T_optimized_option"; 
                 S.List [ S.Atom "elem"; sexp_of_t elem ] ]
    | T_func { params; return } ->
        S.List [ S.Atom "T_func";
                 S.List [ S.Atom "params"; Moon_sexp_conv.sexp_of_list sexp_of_t params ];
                 S.List [ S.Atom "return"; sexp_of_t return ] ]
    | T_raw_func { params; return } ->
        S.List [ S.Atom "T_raw_func";
                 S.List [ S.Atom "params"; Moon_sexp_conv.sexp_of_list sexp_of_t params ];
                 S.List [ S.Atom "return"; sexp_of_t return ] ]
    | T_tuple { tys } ->
        S.List [ S.Atom "T_tuple";
                 S.List [ S.Atom "tys"; Moon_sexp_conv.sexp_of_list sexp_of_t tys ] ]
    | T_fixedarray { elem } ->
        S.List [ S.Atom "T_fixedarray"; 
                 S.List [ S.Atom "elem"; sexp_of_t elem ] ]
    | T_constr x ->
        S.List [ S.Atom "T_constr"; sexp_of_id x ]
    | T_trait x ->
        S.List [ S.Atom "T_trait"; sexp_of_id x ]
    | T_any { name } ->
        S.List [ S.Atom "T_any"; 
                 S.List [ S.Atom "name"; sexp_of_id name ] ]
    | T_maybe_uninit x ->
        S.List [ S.Atom "T_maybe_uninit"; sexp_of_t x ]
    | T_error_value_result { ok; err; id } ->
        S.List [ S.Atom "T_error_value_result";
                 S.List [ S.Atom "ok"; sexp_of_t ok ];
                 S.List [ S.Atom "err"; sexp_of_t err ];
                 S.List [ S.Atom "id"; sexp_of_id id ] ]
