...
let wat_gen_from_module (mod_ : Dwarfsm_ast.module_) =
  let ctx = Dwarfsm_encode_context.make_context () in
  Dwarfsm_encode_resolve.resolve ctx mod_;
  let output_buf = Buffer.create 1000 in
  Buffer.add_string output_buf "(module";
...
let link_core () =
  ...
  let wasm_gen_target sexp =
    ...
    else raise (Arg.Bad ("unrecognized output file type: " ^ filename ^ "; must be one of .wat"))
  in
  ...
  let target =
    match !Config.target with
    | Wasm_gc ->
        Driver_util.Wasm_gc { clam_callback = (fun _ _ -> ()); dwarfsm_callback }
    | Riscv ->
    ...
    try
      ...
      | ast :: [] -> (
          ...
          let postprecess mod_ =
            ...
            in
            let sexp = wat_gen_from_module mod_ in
